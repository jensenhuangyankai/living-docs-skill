# Templates Moved

This skill uses smaller template references to reduce context bloat.

Load only the file you need:

- [design-index.md](templates/design-index.md)
- [architecture.md](templates/architecture.md)
- [service.md](templates/service.md)
- [feature.md](templates/feature.md)
- [ui-pages.md](templates/ui-pages.md)
- [ui-components.md](templates/ui-components.md)
- [changelog.md](templates/changelog.md)
- [build-order.md](templates/build-order.md)
